...
{
    // Si esta apretado, lo unico relevante que puede
    // pasar es que se desaprete
    if(data->pressed)
    {
        if(cqp == PASA_RELEASE)
        {
    ...
    switch (cqp)
    {
        case PASA_RIGHT:
            ++(data->num);
            (data->num) %= GET_NUM_MAX; //incremento sin overflow
            return (suceso_t)(data->num);
        case PASA_LEFT:
            (data->num) ? (data->num)-- : ((data->num) = GET_NUM_MAX - 1);  //decremento sin underflow
            return (suceso_t)(data->num);
        case PASA_PRESS:
            (data->pressed) = true;
            return SUC_N;
        case PASA_RELEASE: case PASA_DELAY: case PASA_ID: case CQP_N:
        default:
            return SUC_N;
    }
}
...
suceso_t get_cursor_move_fun(SUCESO_SOURCE_GET_CURSOR_MOVE_DATA * data, cqp_t cqp)
{
    ...
    // sin antes registrar que se presiono.
    if(!data->pressed)
    {
        if(cqp == PASA_PRESS)
        {
...
void suceso_source__init(SUCESO_SOURCE* self, SUCESO_SOURCE_TYPE type)
{
    ...
    switch (type)
    {
        case SUCESO_SOURCE_GET_NUM_TYPE:
            ((SUCESO_SOURCE_GET_NUM *) self)->type = SUCESO_SOURCE_GET_NUM_TYPE;
            ((SUCESO_SOURCE_ANY *)self)->fun = (get_suceso_fun_p)get_num_fun;
            ((SUCESO_SOURCE_GET_NUM *) self)->data.num = 0;
            ((SUCESO_SOURCE_GET_NUM *) self)->data.pressed = false;
            break;
        case SUCESO_SOURCE_GET_TAP_TYPE:
            ((SUCESO_SOURCE_GET_TAP *) self)->type = SUCESO_SOURCE_GET_TAP_TYPE;
            ((SUCESO_SOURCE_GET_TAP *) self)->fun = (get_suceso_fun_p)get_tap_fun;
            ((SUCESO_SOURCE_GET_TAP *) self)->data.pressed = false;
            ((SUCESO_SOURCE_GET_TAP *) self)->data.tap_detected = false;
            break;
        case SUCESO_SOURCE_GET_CURSOR_MOVE_TYPE:
            ((SUCESO_SOURCE_GET_CURSOR_MOVE *) self)->type = SUCESO_SOURCE_GET_CURSOR_MOVE_TYPE;
            ((SUCESO_SOURCE_GET_CURSOR_MOVE *)self)->fun = (get_suceso_fun_p)get_cursor_move_fun;
            ((SUCESO_SOURCE_GET_CURSOR_MOVE *) self)->data.pressed = false;
            ((SUCESO_SOURCE_GET_CURSOR_MOVE *) self)->data.moving = false;
            break;
        case SUCESO_SOURCE_GET_INC_DISP_INT_TYPE:
            ((SUCESO_SOURCE_GET_INC_DISP_INT *) self)->type = SUCESO_SOURCE_GET_INC_DISP_INT_TYPE;
            ((SUCESO_SOURCE_GET_INC_DISP_INT *)self)->fun = (get_suceso_fun_p)get_inc_disp_int_fun;
            ((SUCESO_SOURCE_GET_INC_DISP_INT *) self)->data.pressed= false;
            break;
        default:
            self->type = SUCESO_SOURCE_TYPE_N;
            ((SUCESO_SOURCE_ANY *)self)->fun = NULL;
    }
}
